var jsPsychSurveyMultiSelect = (function (jspsych) {
  ...
  class SurveyMultiSelectPlugin {
    ...
    trial(display_element, trial) {
      ...
      var slider_id_name = _join(plugin_id_name, "sliders");
      if (trial.sliders !== null) {
        trial_form.innerHTML += '<div id="' + slider_id_name + '" class="' + slider_id_name + '">' + trial.sliders + "</div>";
      }
      ...
      for (var i = 0; i < trial.questions.length; i++) {
        ...
        for (var j = 0; j < question.options.length; j++) {
          ...
          var option_value = question.options[j];
          var option_html = question.options[j];
          if (typeof question.options[j] === 'object' && question.options[j].html !== undefined) {
            option_value = question.options[j].value;
            option_html = question.options[j].html;
          }
          
          label.innerHTML = option_html;
      ...
      display_element.querySelector("#jspsych-survey-multi-select-next").addEventListener("click", () => {
        for (var i2 = 0; i2 < trial.questions.length; i2++) {
          if (trial.questions[i2].required) {
            const checkedInputs = display_element.querySelectorAll(
              "#jspsych-survey-multi-select-" + i2 + " input:checked"
            );
            const requiredSelections = trial.questions[i2].required_selections || 1;
            
            if (checkedInputs.length !== requiredSelections) {
              display_element.querySelector("#jspsych-survey-multi-select-" + i2 + " input").setCustomValidity(
                `Please select exactly ${requiredSelections} option${requiredSelections > 1 ? 's' : ''}`
              );
            } else {
              display_element.querySelector("#jspsych-survey-multi-select-" + i2 + " input").setCustomValidity("");
            }
