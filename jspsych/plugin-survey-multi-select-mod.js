    ...
    parameters: {
      ...
      questions: {
        ...
        nested: {
          ...
          /** If true, then the question is centered and options will be displayed horizontally. */
          horizontal: {
  ...
  class SurveyMultiSelectPlugin {
    ...
    trial(display_element, trial) {
      ...
      if (trial.preamble !== null) {
        trial_form.innerHTML += '<div class="jspsych-survey-multi-select-container"><div id="' + preamble_id_name + '" class="' + preamble_id_name + '">' + trial.preamble + "</div><div class='jspsych-survey-multi-select-questions'></div></div>";
      } else {
        trial_form.innerHTML += '<div class="jspsych-survey-multi-select-container"><div class="jspsych-survey-multi-select-questions"></div></div>';
      }
      var question_order = [];
      ...
      for (var i = 0; i < trial.questions.length; i++) {
        ...
        display_element.querySelector('.jspsych-survey-multi-select-questions').innerHTML += '<div id="' + _join(plugin_id_name, question_id) + '" data-name="' + question.name + '" class="' + question_classes.join(" ") + '"></div>';
        ...
        for (var j = 0; j < question.options.length; j++) {
          ...
          const optionHtml = typeof option === 'object' ? option.html : option;
      ...
      // Add CSS for layout
      const predictionCSS = `
        .jspsych-survey-multi-select-container {
          display: flex;
        ...
        .jspsych-survey-multi-select-preamble {
          width: 100%;
          max-width: 100%;
          padding: 1em;
        ...
        .jspsych-survey-multi-select-preamble > div {
          display: inline-block;
          white-space: nowrap;
        }
        .jspsych-survey-multi-select-preamble table {
          display: inline-block;
          white-space: nowrap;
        }
        .jspsych-survey-multi-select-preamble tr {
          display: inline-block;
          margin-right: 1em;
        }
        .jspsych-survey-multi-select-preamble td {
          display: inline-block;
          vertical-align: middle;
        }
        .jspsych-survey-multi-select-preamble::after {
          content: '';
          position: absolute;
          left: 0;
          right: 0;
        ...
        .jspsych-survey-multi-select-questions {
          flex: 1;
          display: flex;
          flex-direction: column;
          width: 100%;
      ...
      `;
      display_element.querySelector(`#${plugin_id_name}-css`).innerHTML += predictionCSS;

      // Add JavaScript to scroll to the rightmost content
      const preambleElement = display_element.querySelector(`#${preamble_id_name}`);
      if (preambleElement) {
        // Force immediate scroll to the right
        preambleElement.scrollLeft = Number.MAX_SAFE_INTEGER;
      }
      ...
      display_element.querySelector("#jspsych-survey-multi-select-next").addEventListener("click", () => {
        ...
        for (var i2 = 0; i2 < trial.questions.length; i2++) {
          if (trial.questions[i2].required) {
            const checkedBoxes = display_element.querySelectorAll(
              "#jspsych-survey-multi-select-" + i2 + " input:checked"
            );
            const requiredCount = trial.required_selections[trial.questions[i2].name] || trial.required_selections;
            if (checkedBoxes.length !== requiredCount) {
              display_element.querySelector("#jspsych-survey-multi-select-" + i2 + " input").setCustomValidity(
                `Please select exactly ${requiredCount} items (currently ${checkedBoxes.length} selected)`
              );
            } else {
              display_element.querySelector("#jspsych-survey-multi-select-" + i2 + " input").setCustomValidity("");
            }
        ...
        // Check prediction
        const predictionInput = display_element.querySelector('input[name="prediction"]:checked');
        const prediction = predictionInput ? predictionInput.value === 'true' : null;
      ...
      trial_form.addEventListener("submit", (event) => {
        ...
        // Get prediction
        const predictionInput = display_element.querySelector('input[name="prediction"]:checked');
        question_data.prediction = predictionInput ? predictionInput.value === 'true' : null;
