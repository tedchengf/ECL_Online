var jsPsychMultiChoiceIntervention = (function (jspsych) {
	'use strict';
  
	var version = "2.1.0";
  
	const info = {
	  name: "multichoice-intervention",
	  version,
	  parameters: {
		/**
	...
	class SurveyMultiSelectPlugin {
	  ...
	  trial(display_element, trial) {
		var plugin_id_name = "jspsych-survey-multi-select-next";
		...
		display_element.querySelector("#jspsych-survey-multi-select-next").addEventListener("click", () => {
		  for (var i2 = 0; i2 < trial.questions.length; i2++) {
			if (trial.questions[i2].required) {
			  ...
			  if (checkedBoxes.length !== trial.required_selections) {
				display_element.querySelector("#jspsych-survey-multi-select-" + i2 + " input").setCustomValidity(
				  `Please select exactly ${trial.required_selections} items (currently ${checkedBoxes.length} selected)`
				);
			  } else {
				display_element.querySelector("#jspsych-survey-multi-select-" + i2 + " input").setCustomValidity("");
			  }
